...
IDENT_RE = re.compile(
    r'^[\s(]*'                                  # possible leading spaces / opening paren
    r'(?P<name>(?:`[^`]+`|"[^"]+"|\[[^\]]+\]|[A-Za-z0-9_]+)'
    r'(?:\.(?:`[^`]+`|"[^"]+"|\[[^\]]+\]|[A-Za-z0-9_]+)){0,2})'
)
...
# Example usage:
#queries = [
 #   "SELECT COUNT(*) as product_count FROM dev.platinum.api__analytics__products",
  #  "SELECT * FROM platinum.api_analytics_orders",
   # "SELECT * FROM api_analytics_orders",
    #"SELECT a.id FROM mydb.sales.orders a JOIN finance.payments b ON a.id = b.order_id"
#]
...
def get_all_tables_schema_from_database():
    ...
    #with open('tables_schema.json', 'w') as f:
